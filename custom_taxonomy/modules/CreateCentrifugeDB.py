...
class CreateGanonDB(object):
	...
	def __init__(self, database, ganon_database, genomes_path, outdir,verbose=False,processes=1,limit=0,dbprogram="ganon",params=""):
		...
		self.classifyDatabase= ganon_database
		self.ncbi_rewrite_speed = "fastest"
		self.verbose = verbose
		if verbose: print("{classifyDatabase}".format(outdir = self.outdir, classifyDatabase=self.classifyDatabase))
		if not os.path.exists("{classifyDatabase}".format(outdir = self.outdir, classifyDatabase=self.classifyDatabase)):
			os.system("mkdir -p {classifyDatabase}".format(outdir = self.outdir, classifyDatabase=self.classifyDatabase))
	...
	def ganon_fasta_multiproc(self,filepaths,genomes):
		...
		os.system("cat {outdir}/.tmp*.gz > {classifyDatabase}/library.fasta.gz".format(outdir=self.outdir, classifyDatabase=self.classifyDatabase))
		## Rm tmp files
	...
	def create_database(self,outdir,keep=False):
		...
		if not os.path.exists("{ganon}/taxonomy".format(outdir=outdir, ganon=self.classifyDatabase)):
			if self.verbose: print("mkdir -p {ganon}/taxonomy".format(outdir=outdir, ganon=self.classifyDatabase))
			os.system("mkdir -p {ganon}/taxonomy".format(outdir=outdir, ganon=self.classifyDatabase))
		if self.verbose: print("cp {outdir}/*.dmp {ganon}/taxonomy".format(outdir=outdir,ganon=self.classifyDatabase))
		os.system("cp {outdir}/*nodes.dmp {ganon}/taxonomy/nodes.dmp".format(outdir=outdir,ganon=self.classifyDatabase))
		os.system("cp {outdir}/*names.dmp {ganon}/taxonomy/names.dmp".format(outdir=outdir,ganon=self.classifyDatabase))

		if self.verbose: print("ganon build -d {classifyDatabase} --seq-info-file {seqid2taxid} -t {processes} --taxdump-file {classifyDatabase}/taxonomy/nodes.dmp {classifyDatabase}/taxonomy/names.dmp -i {classifyDatabase}/library.fasta.gz".format(classifyDatabase=self.classifyDatabase,seqid2taxid=self.seqid2taxid,processes=self.processes))
		os.system("centrifuge-build -p {processes} --conversion-table {seqid2taxid} --taxonomy-tree {classifyDatabase}/taxonomy/nodes.dmp --name-table {classifyDatabase}/taxonomy/names.dmp {classifyDatabase}/library.fasta.gz {classifyDatabase}".format(classifyDatabase=self.classifyDatabase,seqid2taxid=self.seqid2taxid,processes=self.processes))
